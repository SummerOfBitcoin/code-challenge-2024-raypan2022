...
# Assume you have a function to receive or fetch new transactions
...
MAX_BLOCK_SIZE = 1000000  # Simplified maximum block weight in weight units (actual Bitcoin block size limit is 4 MB in weight units)

def calculate_total_bytes(tx):
    """Calculate the total bytes of a transaction.
    This function assumes that the transaction structure includes fields from the given JSON structure.
    We'll serialize the transaction and calculate the length of the resulting JSON string."""
...
def calculate_vsize(tx):
    """Calculate the virtual size of a transaction with consideration for SegWit discount."""
